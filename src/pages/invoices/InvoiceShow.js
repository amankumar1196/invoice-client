...
import { showInvoicePreview } from '../../redux/actions/invoiceActions';

function PreviewInvoice(props) {
  const { dispatch, isShowInvoicePreview, globalFormValues } = props;
  ...
  return (
    <ModalPortal>
      <div class="invoice-preview-modal">
        <div class={`modal ${ isShowInvoicePreview ? "show" : "fade"}`} id="editBudgetModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-modal="true">
          <div class="modal-dialog modal-lg">
            <div class="modal-content">
              <div class="close mr-8" onClick={() => dispatch(showInvoicePreview(false))} data-dismiss="modal" aria-label="Close"><i class="bx bx-x text-neutral-900 font-size-24"></i></div>
...
function mapStateToProps(state) {
  const { invoice } = state;
  return {
    isShowInvoicePreview: invoice.isShowInvoicePreview
  };
}
